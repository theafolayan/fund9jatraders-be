    ...
    protected function getFormSchema(): array
    {
        return [
            ...
            Card::make()->label("Product One Settings")->schema([
                ...
                TextInput::make('product_one_price')->label('Product One Price')->mask(fn (TextInput\Mask $mask) => $mask->money(prefix: '₦')),
                MarkdownEditor::make('product_one_description')->label('Product One Description'),

            ]),
            Card::make()->label("Product Two settings")->schema([
                TextInput::make('product_two_title')->label('Product Two Title'),
                TextInput::make('product_two_price')->label('Product Two Price')->mask(fn (TextInput\Mask $mask) => $mask->money(prefix: '₦')),
                MarkdownEditor::make('product_two_description')->label('Product Two Description'),
            ]),

            Card::make()->label("Product Three settings")->schema([
                TextInput::make('product_three_title')->label('Product Three Title'),
                TextInput::make('product_three_price')->label('Product Three Price')->mask(fn (TextInput\Mask $mask) => $mask->money(prefix: '₦')),
                MarkdownEditor::make('product_three_description')->label('Product Three Description'),
